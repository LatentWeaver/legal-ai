...
- **Download:** [master_citations_cases.db.gz (62 MB) — Google Drive](https://drive.google.com/file/d/1d5LTT3KzKzqPJt4-WfsN9XMEV0cCnPhz/view?usp=sharing)
...
- **Phase 2 — COMPLETE (2026-06-11):** full content of **every** cited / citing case scraped — **71,998 / 71,998 (100.0%)** unique related cases (statutes excluded). Every scraped case traces back to a master's citation list (verified 100%), avg judgment length ~68K chars, plus **1,293,437 inline citations with sentiment** captured as a parse byproduct. Output `related_cases_cases.db` (14 GB SQLite → 3.0 GB gzip; same `cases` schema as the master DB, minus the search tables):
  - **Download:** [related_cases_cases.db.gz (3.0 GB) — Google Drive](https://drive.google.com/file/d/1oaYI3mk4AsX1w8GCnIFRQ-80Du5rh1v7/view?usp=share_link)
  - **Restore:** `gunzip related_cases_cases.db.gz` → query with any SQLite client.
- **Team-standard citation file:** the normalized `{case, year, url, precedents[{case,url}]}` version of this dataset lives on `main` at `citations-data/6001-6750/citations.json` (750 cases, 25,012 precedent entries), generated by [`scraper/to_standard_format.py`](scraper/to_standard_format.py).
