"""
Fetch citations for rows 5251-6000 from the Indian Kanoon API.

Outputs:
  - citations_progress_5251_6000.jsonl  — one JSON line per processed doc (checkpoint, auto-resumed)
  - citations_output_5251_6000.csv      — final flat CSV (one row per citation)
  - citations_summary_5251_6000.csv     — one row per case with metadata + citation count

Run:
  python fetch_citations_5251_6000.py           # full run (rows 5251-6000)
  python fetch_citations_5251_6000.py --export  # skip fetching, just export existing progress to CSV
"""
...
ROW_START = 5250        # 0-indexed (= row 5251 in the sheet)
ROW_END   = 6000        # exclusive
DELAY     = 1.0         # seconds between requests
MAX_RETRY = 3           # retries on transient errors
RETRY_BACKOFF = 5       # seconds to wait on first retry (doubles each time)

PROGRESS_FILE  = "citations_progress_5251_6000.jsonl"
OUTPUT_FLAT    = "citations_output_5251_6000.csv"
OUTPUT_SUMMARY = "citations_summary_5251_6000.csv"
